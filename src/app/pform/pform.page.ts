...
@Component({
  selector: 'app-pform',
  templateUrl: './pform.page.html',
  styleUrls: ['./pform.page.scss'],
})
export class PformPage implements OnInit {
  public user: User;
  ...
  async createForm() {
    ...
    this.firestoreService
      .createProfForm(pName, pEmail, sTime, eTime,pId,sId,sName,studentId,sEmail,createdAt,accepted,reason)
      .then(
        () => {
          loading.dismiss().then(() => {
            this.router.navigateByUrl('/pmeeting');
          });
        },
        error => {
          loading.dismiss().then(() => {
