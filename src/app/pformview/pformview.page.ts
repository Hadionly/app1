...
@Component({
  selector: 'app-pformview',
  templateUrl: './pformview.page.html',
  styleUrls: ['./pformview.page.scss'],
})
export class PformviewPage implements OnInit {
  public pform: Pform;
  ...
  ngOnInit() {
    ...
    this.firestoreService.getPformDetail(userId).subscribe(pform => {
      this.pform = pform;
    ...
    const alert = await this.alertController.create({
      ...
      buttons: [
        ...
        },
        {
          text: 'Okay',
          handler: () => {
            this.firestoreService.deletePform(userId).then(() => {
              this.router.navigateByUrl('/viewstudentmeeting');
            });
          },
        },
      ],
 ...
 async accepted(){
    ...
    this.db2.collection(`pform`).doc(this.pform.id).update({ accepted: true/*NEW QUANTITY*/ });
    let event = {
      title: title,
      start: start,
      end: end,
  ...
  async decline(){
    const loading = await this.loadingCtrl.create();
    this.db2.collection(`pform`).doc(this.pform.id).update({ accepted: false/*NEW QUANTITY*/ }).then(
      () => {
        loading.dismiss().then(() => {
          this.router.navigateByUrl('/viewmeeting');
        });
